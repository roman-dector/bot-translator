...
from utils import is_valid_user, save_to_db
        ...
        for phrase, transcription, audio_link in data:
            ...
            with tempfile.NamedTemporaryFile() as f:
                ...
                query.message.reply_voice(
                    ...
                    caption=f"{phrase} [{transcription}]",
                )
...
@is_valid_user
def save_to_favorites(update: Update, context: CallbackContext) -> None:

    query = update.callback_query
    query.answer()

    word, definition = query.message.text.split("\n", 1)[0][len("Word: "):], query.message.text

    save_to_db(word, definition)

    query.message.reply_html(f'The definition of "{word}" was saved to favorites')
